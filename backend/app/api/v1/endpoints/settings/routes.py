...
async def update_display_settings(
    ...
    try:
        ...
        if not settings:
            settings = UserSettingsModel(
                user_id=current_user.id, display_settings={}, map_settings={}
            )
            db.add(settings)
...
async def get_appearance_settings(
    ...
    return {
        "code": 200,
        "message": "success",
        "data": {"theme": settings.theme, "font": settings.font},
    }
...
async def get_llm_settings(
    ...
    if not settings or not settings.llm_config:
        return {"code": 200, "message": "success", "data": LLMConfig()}

    return {"code": 200, "message": "success", "data": LLMConfig(**settings.llm_config)}


# 更新LLM配置
@router.put("/llm", response_model=Response[LLMConfig])
